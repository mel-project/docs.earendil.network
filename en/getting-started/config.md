...
```yaml
# [optional] path to file for storing information that's persisted across reboots of `earendil` daemon (e.g., the global relay graph and chat history). This must be a path writable by the `earendil` daemon. Storing this information may speed up initial connection times. 
db_path: ./.cache/earendil

# [include for relays, omit for clients] path to file for storing a long-term identity.
...
# starts a local Socks5 server that proxies traffic through Earendil. This gives you access to Earendil havens.
socks5:
