...
客户端通过聊天、电子邮件或任何必要的方式来获取中继的联系信息，这些信息默认不公开，以确保**抗封锁**：如果任何客户端都可以请求完整的中继 IP 地址列表，那么审查者可以简单地加入网络，请求所有中继的联系信息，并将所有中继的 IP 地址列入黑名单。您可以在[关于](https://docs.earendil.network/)部分阅读更多关于 earendil 如何实现抗封锁的信息。

因此，与上面的 `example-relay` 不同，您需要四处询问以获取更多中继的访问权限。欢迎加入[我们的 Discord](https://discord.gg/jdVuk4Qj89)以获取更多中继信息！
